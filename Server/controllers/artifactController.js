...
exports.updateDocType = async (req, res) => {
    const { id } = req.params;
    const { docTypeName, initalDocType } = req.body;
    try {
        await documentService.updateDocType(id, docTypeName);
        await authService.logActivity(req.id, `User: ${req.email} updated a document type from ${initalDocType} to ${docTypeName}`);
...
exports.addNewDocType = async (req, res) => {
    ...
    try {
        ...
        await authService.logActivity(req.id, `User: ${req.email} added a new document type: [${newDocTypeName}]`);
...
exports.deleteDocType = async (req, res) => {
    ...
    try {
        ...
        await authService.logActivity(req.id, `User: ${req.email} deleted a document type: [${docTypeName}]`);
...
exports.deleteArtifact = async (req, res) => {
    ...
    try {
        ...
        await authService.logActivity(req.id, `User: ${req.email} deleted a document: [${docName}]`);
