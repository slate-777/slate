...
exports.uploadDocument = async (req, res) => {
    upload.single('file')(req, res, async (err) => {
        ...
        const documentData = {
            ...
            isPublished, email: req.email, fileSize
        };
...
exports.addUrl = async (req, res) => {
    ...
    const documentData = {
        infoHead, ownerAuthorId, url, docType, assocTags, description, isPublished, email: req.email
    };
    try {
...
exports.updateDocumentMetadata = async (req, res) => {
    const { id } = req.params;
    const { tags, docName, docType, description, publish, status } = req.body;
    const assocTags = tags.join(','); // Convert tags array to a comma-separated string
    const metadata = {
    ...
    try {
        ...
        await authService.logActivity(req.id, `User: ${req.email} updated a document: [${docName}]`);
