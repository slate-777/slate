...
class UploadService {
    async uploadDocument(documentData) {
        const { docName, ownerAuthorId, filePath, docType, docFormat, tags, description, isPublished, email, fileSize } = documentData;
        const query = `INSERT INTO documents 
                       ...
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?, ?)`;
        const [result] = await db.promise().query(query, [docName, ownerAuthorId, filePath, docType, docFormat, tags, description, 'active', isPublished, email, fileSize]);
        return result;
    }

    async addUrl(documentData) {
        const { infoHead, ownerAuthorId, url, docType, assocTags, description, isPublished, email } = documentData;
        const query = `INSERT INTO documents 
                       ...
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?)`;
        const [result] = await db.promise().query(query, [infoHead, ownerAuthorId, url, docType, 'url', assocTags, description, 'active', isPublished, email]);
        return result;
    }
