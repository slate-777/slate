...
class ArtifactsService {
    ...
    async fetchCountOfArtifacts() {
        const [results] = await db.promise().query(`
        SELECT 
            SUM(CASE WHEN doc_format != 'url' AND is_published = 1 THEN 1 ELSE 0 END) AS total_docs,
            SUM(CASE WHEN doc_format = 'url' AND is_published = 1 THEN 1 ELSE 0 END) AS total_urls
        FROM vw_documents
    `);
        return results[0];
    };
